...
## (Haskellのライブラリなので)<br/>Scalaに移植してみよう！
...
- TSUYUSATO Kitsune (@MakeNowJust)
- 普段はTypeScriptとかを書いている
- Crystalというプログラミング言語のコントリビュータ
- 形式言語が好き
...
- SmallCheckのような感じで、小さいものから順番にチェックしていく方針
- ランダムにデータを生成するのは*サポートしていない*
- Genericを使ったデータを生成するクラスの自動導出
- Template Haskellを使ったデータを生成する型クラスの自動導出
- **関数の生成をサポートしている**
...
(leancheck4sという名前が<br/>当時はなぜか思い付かなかった‥‥)
...
- property-based testingの基本的なサポート
- Magnoliaによる生成と表示を担当する型クラスの自動導出
...
- `class WithInspectConfig[A]`:<br/>`InspectConfig`を持ったReader Monad
...
- `Stream`は難しすぎる。<br/>すぐにOOMとStackOverflowを起こす
- Magnoliaの黒魔術感‥‥。
- テストはμTestとsbt-doctestで書いた。<br/>doctest良い
- ドキュメントはmdocとDocusaurusで作った。<br/>
